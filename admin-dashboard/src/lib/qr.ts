...
export function barcodeImageUrl(text: string, format: string, width = 240, height = 120): string {
  const type = barcodeTypeMap[format] ?? 'code128';
  if (type === 'qrcode') {
    return qrCodeUrl(text, Math.min(width, height * 2));
  }
  return `https://quickchart.io/barcode?type=${encodeURIComponent(type)}&text=${encodeURIComponent(text)}&width=${width}&height=${height}&format=png`;
}
