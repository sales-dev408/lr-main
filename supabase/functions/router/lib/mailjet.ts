...
export async function sendVendorWelcomeEmail(input: VendorWelcomeEmailInput): Promise<void> {
  ...
  const auth = btoa(`${config.mailjetApiKey}:${config.mailjetSecretApiKey}`);
...
async function sendEmailBatch(subject: string, text: string, html: string | undefined, toList: string[]): Promise<number> {
  if (!config.mailjetApiKey || !config.mailjetSecretApiKey) {
...
async function sendSms(text: string, to: string): Promise<void> {
